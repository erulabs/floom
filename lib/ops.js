 ...
 * @returns {Stream}
 */
Opsjs.prototype.exec = function (cmd, onComplete, onError, onLine, onData) {
  var self = this;
  self.log.debug('ops.exec(): Attempting to execute on all nodes');
  return self.simple(function (node, callback) {
    function fire () {
      node.exec(cmd, function (data, code, signal) {
        //self.log.debug('node.exec('+node.data.name+'): \n', data);
        if (onComplete) {
          onComplete(data, code, signal);
        }
        callback(null, node.data.name);
      }, onError, onLine, onData);
    }
