...
Opsjs.prototype.simple = function (cb) {
  ...
  stream._transform = function (obj, encoding, streamcb) {
    cb(obj.toString(), function () {
      streamcb(null, obj);
    });
  };
  return stream;
...
/**
 * Retrieve the node object for every node passed thru, extension of .simple()
 * @param {function} cb - A callback which is fired with (nodeObject, callback)
 * @returns {Stream}
 */
Opsjs.prototype._nodeCaller = function (cb) {
  var self = this;
  return self.simple(function (node, streamcb) {
    var ourNode = self._nodes[node];
...
Opsjs.prototype.exec = function (execCode) {
  ...
  return self._nodeCaller(function (node, cb) {
...
Opsjs.prototype.save = function () {
  ...
  return this._nodeCaller(function (node, cb) {
    node.save(function () {
      cb();
    });
  });
