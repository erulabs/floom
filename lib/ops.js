 ...
 * @returns {Stream}
 */
Opsjs.prototype.exec = function (cmdObj, events) {
  var self = this;
  var dispatches = 0;
  ...
  if (typeof events === 'function') {
    events = { onComplete: events };
  }
  return self.simple(function (node, callback) {
    self.log.debug('ops.exec(): Attempting to execute on ' + node.data.name + ' call ' + dispatches++);

    events._node_exec_complete = function () {
    ...
    node.exec(cmdObj.command, events);
  });
};
 ...
 * @returns {Stream}
 */
Opsjs.prototype.package = function (packages, events) {
  var self = this;
  var dispatches = 0;
  ...
  return self.simple(function (node, callback) {
    self.log.debug('ops.package(): Attempting to execute on ' + node);
    packages.forEach(function (pkg) {
      ...
      node.exec('sudo apt-get install -y '+pkg, events);
    });
  });
};
